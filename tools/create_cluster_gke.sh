...
gcloud beta container --project $GCP_PROJECT clusters create $CLUSTER --region $GCP_REGION --no-enable-basic-auth --release-channel "rapid" --machine-type "n1-standard-4" --image-type "UBUNTU" --disk-type "pd-ssd" --disk-size "150" --preemptible --num-nodes "2" --enable-autoscaling --min-nodes "1" --max-nodes "15" --enable-autoupgrade --enable-autorepair --no-enable-stackdriver-kubernetes --disable-addons HttpLoadBalancing
