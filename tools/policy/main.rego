...
deny[msg] {
  ...
  msg = sprintf("Please set [app] [role] pod template label and selector (spec.template.metadata.labels.app, spec.selector.matchLabels.app): [Resource=%s, Name=%s]", [input.kind, input.metadata.name])
}

deny[msg] {
  not input.metadata.labels.app
  not input.metadata.labels.role
  msg = sprintf("Please set [app] [role] labels (metadata.labels): [Resource=%s, Name=%s]", [input.kind, input.metadata.name])
}
